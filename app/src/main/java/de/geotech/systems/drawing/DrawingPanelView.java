...
public class DrawingPanelView extends SurfaceView implements SurfaceHolder.Callback {
	...
	private void updateMapViewZoom() {
		...
		if (osmMapView.getVisibility() == View.VISIBLE) {
			...
			// get corner coordinates of map projection
		 	org.osmdroid.views.Projection proj = osmMapView.getProjection();
			IGeoPoint ul_proj = proj.fromPixels(0, 0);
			IGeoPoint lr_proj = proj.fromPixels(mapScreen.width, mapScreen.height);
	...
	private void setBackgroundMap(MapView osm)
    {
		...
		// offline tile database - Hadi
		this.osmMapView.setTileSource(new XYTileSource("examplez", 2, 15, 256, ".png", new String[] {}));
	}
